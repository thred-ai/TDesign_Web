    ...
    Ownable
{
    ...
    function mintAndTransfer(NFTVoucher calldata voucher)
        public
        payable
        nonReentrant
        returns (bool)
    {
        ...
        address signer = redeem(msg.sender, msg.value, voucher);
        return createMarketItem(payable(signer), payable(msg.sender), voucher);
    }

    function createSale(MarketItem calldata item)
        public
        payable
        nonReentrant
        returns (bool)
    {
        ...
        require(msg.value == item.price, "Price must match");
        require(
